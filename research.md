...
- **Rückmeldung PH-Ökogarten — 2 von 6 Zielarten zuverlässig vorhanden.** Verfügbar im Ökogarten: Pastinaca sativa (zahlreich auf der Wiese gegenüber INF 561) und Daucus carota — beide aber Anfang Mai noch nicht blühend, frühestens Juli/August. Bonus: Angelica (Engelwurz) ist dort zu finden — nicht in unserem 6-Klassen-Zielsatz, aber als zusätzliche Apiaceae für ein Out-of-Distribution-Test interessant (was sagt das Modell zu einer Doldenblütler-Gattung, die es nicht kennt?). Conium maculatum gibt es im Ökogarten nicht (braucht Gewässer-Habitat), Heracleum sphondylium ebenfalls nicht (eher am Neckar zu finden). Anthriscus sylvestris und Aethusa cynapium nicht erwähnt. Konsequenz für die Sammelplanung: (a) Ökogarten-Termin ab Mitte/Ende Juli für Pastinake + Wilde Möhre + Engelwurz, (b) Conium und Heracleum müssen außerhalb des Ökogartens gefunden werden — Neckarumfeld bzw. Gewässerränder, (c) Anthriscus weiterhin im Freiland, idealerweise jetzt noch in Blüte. Mehrere Sammeltermine sind damit unausweichlich, statt eines einzigen Sommer-Termins.
- **Schärfung des Frucht-Befunds — Blüten in Apiaceae sind merkmalsarm.** Zweite Folgemail vom Botanischen Garten präzisiert den gestrigen Hinweis: Apiaceae sind eine "homogene Familie", **alle Arten sehen blühend fast gleich aus**; die diagnostisch tragenden Merkmale sitzen strukturell in den Früchten. Erfahrene Floristen schauen sich bei dieser Familie oft *nur* die Früchte an. (Vergleichbares Muster bei Brassicaceae.) Konsequenz für unser Projekt: unser gesamter Stack — Synth-Generator, Render-Pipeline, Synth-CNN, Leaf-CNN — arbeitet auf dem **schwächeren** der beiden Merkmalskomplexe. Das Conium↔Anthriscus-Problem aus den frühen v3/v4/v5-Iterationen (76 % aller Test-Fehler entfielen auf dieses Paar) ist damit kein zufälliger Architektur-Bug, sondern eine **systematische Decke** für alle Arten-Paare, deren Diskriminanz primär in der Frucht liegt — keine ML-Architektur kann ein Signal extrahieren, das im Eingangsbild prinzipiell nicht vorhanden ist. Drei Schlussfolgerungen: (a) Der gestrige "single-feature-complex"-Reframe bleibt gültig, aber mit Caveat: für manche Arten-Paare hat unser Ansatz eine harte obere Schranke; eine ehrliche Veröffentlichung müsste das offen ausweisen statt es als Architektur-Limit zu kaschieren. (b) Optimistisches Gegenargument bleibt empirisch offen: ML kann subpixel-/farbverteilungs-/textur-Signale ausnutzen, die Botaniker mit bloßem Auge nicht ausmachen — "merkmalsarm für Menschen" ist nicht zwingend "merkmalsarm für ein CNN". Diese Frage entscheidet sich erst auf einem ausreichend großen Real-Foto-Set. (c) Ein Frucht-Branch-Modell (vorher als nice-to-have eingestuft) rückt damit auf der Prioritätenliste nach oben — er ist nicht nur Ergänzung, sondern für die schwer trennbaren Paare möglicherweise *die* eigentliche Architektur, von der ein Blüten-Branch nur abweicht.

### 2026-05-07 — Fünfte Session

- **Mehrere Design-Korrekturen aus botanischer Drittmail.** Dritter Folgehinweis vom Botanischen Garten bringt fünf Punkte mit direkten Konsequenzen für unser Vorgehen: (1) **App-Baseline plausibel.** Anekdotische Beobachtung, dass PlantNet/Flora-Incognita-artige Tools auch auf Apiaceae "erstaunlich gut" funktionieren — heißt für uns: sobald ein Real-Foto-Test-Set existiert, müssen wir gegen mindestens zwei kommerzielle 2D-CNN-Apps benchmarken, sonst ist die Forschungsfrage "bringt unsere Methode einen Vorteil?" nicht ernsthaft beantwortbar. (2) **Bestimmung ist Merkmals-Kombi, nicht single-feature.** Korrigiert die gestrige Verkürzung "Floristen schauen nur Früchte an" — auch sie integrieren implizit mit Habitus, Blattform, Standort etc. Heißt für uns: das Frucht-Branch-Argument bleibt gültig, aber selbst ein "perfektes" Frucht-Modell wäre keine Komplettlösung; ein guter Klassifikator integriert mehrere Merkmalskomplexe. (3) **Grundblätter vs. Stängelblätter ist diagnostisch relevant** — unser Leaf-CNN trainiert auf iNat-Fotos ohne diese Differenzierung. Das ist eine ungenannte Annahme in unserem Datensatz, die das Modell vermutlich verschmiert. Bei nächster Iteration des Leaf-CNN sollte geprüft werden, ob iNat-Fotos taggable nach "Grundblatt vs. Stängelblatt" sind, oder ob das Crop-Heuristisch (untere 20% der Pflanze) angenähert werden kann. (4) **"Nie nur ein Exemplar mitnehmen"** — botanische Sammelpraxis übersetzt direkt in eine Test-Set-Designforderung: pro Art mehrere *verschiedene Individuen* an verschiedenen Standorten/Tagen, nicht 50 Fotos vom selben Exemplar. Genauso wie unsere `instance_stratified_split`-Logik für synthetische Daten — selbe Disziplin auch real anwenden. (5) **Konkreter Expert-Lead für später:** Birgit Nordt (BGBM Berlin), Bearbeiterin des Apiaceae-Kapitels im aktuellen Schmeil-Fitschen — nicht jetzt kontaktieren (ohne konkrete Frage wäre das verschwendete Aufmerksamkeit), sondern wenn wir ein zielgerichtetes Anliegen haben, etwa: Frucht-Branch in Planung + Frage nach existierender DACH-Frucht-Bildreferenz, oder Validierung unserer L-System-Parameter gegen eine Spezialistinnen-Sicht. Plus eine persönliche Empfehlung an mich: selbst einmal klassisch mit einem Bestimmungsbuch (Schmeil-Fitschen oder Rothmaler) Apiaceen im Feld bestimmen, um ein Gefühl für die analoge/variable Beschaffenheit der Merkmale zu bekommen — Intuition, die einem ML-Modell-Bauer fehlt, der nur mit synthetischen Daten arbeitet.
- **Erfolgsmaßstab neu eichen + neuer Atlas-Lead.** Vierte Folgemail vom Botanischen Garten enthält drei Punkte, die unseren Bewertungsmaßstab und unsere Render-Strategie betreffen: (1) **"Vermutlich eine dieser n Arten" ist ein botanisch legitimes Ergebnis**, kein Modell-Versagen — gerade in schwierigen Gruppen ist eine eindeutige Ein-Arten-Antwort aus einem einzigen Habitusfoto manchmal prinzipiell nicht erreichbar. Konsequenz: unser bisheriger Erfolgsmaßstab (Top-1-Genauigkeit als zentrale Kennzahl) ist überzogen; **Top-k mit kalibrierter Unsicherheit** ist die botanisch angemessene Zielgröße, gerade für Conium↔Anthriscus & co. Die Top-3-Spalten in unseren bisherigen Eval-Tabellen waren halb-bewusst als Trostpreis behandelt; sie sollten als Hauptmetrik aufgewertet werden. (2) **Klassische Bestimmungsschlüssel selbst erreichen kein 100 %** auf realen Exemplaren — Merkmalsvariation und -überlappung machen das in vielen Untergruppen unmöglich. Heißt für unser Forschungsdesign: ein 100-%-Ziel ist nicht nur unerreichbar, sondern wäre verdächtig (Overfit oder Test-Leak); das gilt sogar auf Synth-Test (v6 hatte 98.2 %, v7 ~99 % — möglicherweise schon ein Indikator für Render-Artefakte statt botanischer Generalisierung, was konsistent mit der hier dokumentierten Synth-Acc/Real-Acc-Entkopplung ist). (3) **Konkreter neuer Lead: Rothmaler Atlasband** (Müller/Ritz/Welk) — wissenschaftliche Strichzeichnungen mit Pfeilen auf die diagnostischen Merkmale, im Feld auch von erfahrenen Botanikern als Schnell-Vergleich verwendet, statt einen vollen Bestimmungsschlüssel durchzugehen. Doppelter Wert für uns: (a) als Lerngrundlage für meine eigene Apiaceae-Bestimmung im Sommer; (b) als potenzielle Eingabe für den Synth-Generator — wenn die Atlas-Pfeile explizit markieren *welche* Merkmale die diskriminanten sind (z.B. "bei Aethusa: reflexed Bracteoles am Umbellet-Boden"), können wir gezielt diese Merkmale in der Render-Pipeline überbetonen, statt undifferenziert die ganze Pflanze zu rendern. Letzteres ist aktuell als Hypothese zu sehen — ob ein botanisches Atlasband mit Pfeil-Annotationen tatsächlich gut auf eine ML-Render-Featureauswahl übersetzbar ist, müsste man im Detail prüfen.
- **Architektur-Entscheidung: Leaf-Branch zurück in den 3D-Synth-Pfad.** Selbstkritische Beobachtung des Users: wenn wir das Projekt um Leaf- und Fruit-Branches erweitern und beide auf 2D-Real-Fotos setzen, dann ist das L-System-/Render-Pipeline-/Synth-CNN-Stück nur noch *einer von drei* Branches — ausgerechnet auf dem botanisch schwächsten Merkmalskomplex (siehe vorigen Eintrag). Das ursprüngliche Forschungsthema "2D→3D-Brücke verbessert Bestimmung" würde damit von Kern-Beitrag zu Minderheits-Komponente. **Gegenmaßnahme: Leaf-Branch wird nicht als 2D-Foto-CNN gebaut, sondern als prozeduraler 3D-Pfad analog zum Inflorescence-Branch.** Botanisch passt das gut: Apiaceae-Blätter sind klassisch 2-bis-3-fach gefiedert, also genau die rekursive Verzweigungs-Topologie, für die L-Systeme gemacht sind. Das diskriminative Signal zwischen Arten sitzt im Wesentlichen in dieser Topologie ("2-3-fach gefiedert mit schmal lanzettlichen Endabschnitten"), also strukturell — was in einem Skelett-+-Outline-Modell abbildbar ist. **Modell-Skizze:** Leaf-Skelett aus L-System (Mittelrippe → Seitennerven → Tertiärnerven, rekursiv), parametrische Outline-Kurve in variablem Offset-Abstand vom Skelett-Endpunkt-Pfad (definiert die Blattrand-Form), Dicke als Skalar fürs Shading. **Bewusste Auslassungen für die erste Iteration:** Behaarung, Wachsbelag, Verfärbungen, Insektenfraß — können in späteren Iterationen analog zu den Render-Augmentation-Komponenten ergänzt werden. **Bewusste Akzeptanz der Synth→Real-Gap:** der Verlust an realer Variation gegenüber iNat-Fotos ist gerade der Punkt — das Projekt will testen, ob prozedurale Daten zur Real-Foto-Klassifikation reichen, und nicht den Test umgehen. Außerdem: das aktuelle Leaf-CNN-Setup vergleicht real-Foto gegen real-Foto, weil iNat-Research-Grade-Fotos in der Regel keine gezielten Blatt-Aufnahmen sind, sondern Habitus-/Mischfotos. Damit ist der bisherige 76%-Wert des Leaf-CNN ohnehin nicht "leaf-spezifisch" — der Vergleich gegen einen echten prozeduralen Leaf-Klassifikator wäre der erste sauber geführte Vergleich. **Nicht-Entscheidung:** Fruit-Branch bleibt vorerst draußen. Mericarp-Diagnostik sitzt auf 2-5 mm Skala (Riefen, Flügel, Borsten), bei normalem Smartphone-Foto-Auflösung sub-pixel — derselbe Auflösungs-Limit-Mechanismus wie bei der Photogrammetrie (Eintrag 04.05.). Bevor das angegangen wird, müsste eine Macro-Crop-Render-Strategie konzipiert werden; eigenes Folge-Forschungsthema, jetzt nicht. **Nächster Schritt:** Prototyp eines Anthriscus-Leaf-Skeletts via L-System, plus naive Outline-Funktion, ohne Klassifikator — Augenmaß-Validierung, ob das Render botanisch plausibel aussieht. Wenn ja, auf alle 6 Arten ausweiten und gegen das alte iNat-Leaf-CNN benchmarken.

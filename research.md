...
- **Photogrammetrie-Versuch an realer Apiaceae gescheitert — Ursache präzisiert** (Scaniverse + ~50 Photo-Mode-Aufnahmen einer realen Wiesen-Kerbel-Pflanze in Vase): das resultierende Mesh enthielt **nur die Vase** als Volumen-Geometrie. Die Pflanze selbst (Stamm, Strahlen, Pedicels, Blätter) wurde nicht als 3D-Struktur rekonstruiert — stattdessen als 2D-Textur auf die Wandflächen des Hintergrunds projiziert. Klassisches "see-through depth ambiguity"-Versagen von Structure-from-Motion.
  ...
  **Was technisch möglich wäre (für Vollständigkeit):** Multi-Scale-Capture — separater Macro-Scan vom Doldenkopf aus 15–25 cm Distanz (für Pedicel-Auflösung) plus separater Standard-Scan vom Stamm aus 50–80 cm (für Stamm-Geometrie), beide nachträglich registriert. Aufwand höher, aber prinzipiell innerhalb der Photogrammetrie-Möglichkeiten. Alternativ: strukturiertes Licht (Revopoint MINI 2 ~800 €), das mit Specular-Tracking arbeitet statt mit Pixel-Korrespondenzen und das angulare-Limit umgeht.
...
- **Hybrid-Inferenz Synth + Leaf, asymmetrische Gewichtung schlägt naives Mitteln** (`src/inference/hybrid.py`, `notebooks/26_hybrid_eval.py`): Wrapper-Modul lädt beide Modelle, läuft beide auf jedem Bild, kombiniert Wahrscheinlichkeiten. Fünf Strategien evaluiert auf den 9 Real-Fotos (Stockfoto + 4 Indoor + 4 Outdoor): naives Soft-Vote 50/50 schadet (4/9 Top-3 vs 6/9 für Synth allein), weil Synths Pastinaca-Überkonfidenz auf Out-of-Distribution-Bildern die Leaf-Predictions kontaminiert. **Soft 30/70 (Synth 30%, Leaf 70%) gewinnt** mit **2/9 Top-1-Treffer** — sowohl der Synth-Stärke (Indoor Stamm) als auch der Leaf-Stärke (Outdoor Blatt) gleichzeitig. Erstmals besser als jedes Einzelmodell. Andere Strategien (Confidence-Selection, Leaf-Priority, Vote) brachten keine zusätzlichen Top-1-Treffer; sie haben aber besseres Top-3 als Soft 30/70 (5–6/9). Erkenntnis: **Leaf ist die zuverlässigere Basis für Real-Fotos** (real-trainiert), Synth liefert *gezielte* Diagnostik (z.B. fehlende Stamm-Speckles → Anti-Conium-Signal); 30/70 hält beides ohne Verwässerung. Ehrlich: 2/9 ist Fortschritt aber nicht Produktionsreife — 6/9 Fotos bleiben falsch. Hauptbottleneck weiterhin Bilder die nur Doldenkopf zeigen oder Ganzpflanzen-Übersichten.

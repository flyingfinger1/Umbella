"""20: build v6 (augmented) and train on it back-to-back.

Identical training hyperparameters to v5 — only the dataset changes (now with
natural backgrounds, Lambert shading, color jitter). Lets us isolate the
augmentation effect.

Output goes to data/models/v6_log.txt.
"""
...
OUT_CKPT = ROOT / "data" / "models" / "v6_classifier.pt"
LOG = ROOT / "data" / "models" / "v6_log.txt"
...
def main():
    t_global = time.time()
    log(f"=== v6 overnight {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n")

    # build dataset
    log("=== STEP 1: building v6 (augmented) ===")
    t0 = time.time()

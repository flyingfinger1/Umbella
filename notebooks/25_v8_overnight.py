"""25: build v8 (synth + 50% iNat backgrounds) and train.

Same hyperparameters as v6/v7. Only the augmentation backgrounds change
between v7 and v8 datasets. Hypothesis: real-photo classification on
outdoor backgrounds should improve, because the model has now seen
photo-realistic backgrounds during training.
"""
...
DATA = ROOT / "data" / "training" / "v8"
INAT_BG = ROOT / "data" / "leaf_images"
OUT_CKPT = ROOT / "data" / "models" / "v8_classifier.pt"
LOG = ROOT / "data" / "models" / "v8_log.txt"
...
def main():
    t_global = time.time()
    log(f"=== v8 overnight {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n")

    log("=== STEP 1: building v8 (with iNat backgrounds) ===")
    t0 = time.time()
    build_dataset(out_root=DATA, n_per_species=200, n_views=4,
                  image_size=(384, 384), augment=True,
                  inat_bg_dir=INAT_BG, inat_bg_prob=0.5,
                  progress=False)
    log(f"  build done in {(time.time() - t0) / 60:.1f} min\n")

    log("=== STEP 2: training v8 ===")
